...
import type { ProfitLossMessages } from "@/lib/i18n/profit-loss-messages";
import { formatRangeLabel } from "./profit-loss-ui";

type ProfitLossDateRangeSheetProps = {
  open: boolean;
  from: string;
  to: string;
...
export default function ProfitLossDateRangeSheet({
  open,
  from,
  to,
  labels,
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-t-2xl bg-white px-4 pb-6 pt-4 shadow-2xl">
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="text-[15px] font-medium text-[#333]">{monthLabel(viewMonth, locale)}</span>
          <button
            type="button"
            className="rounded-md px-2 py-1 text-[#2196F3]"
            onClick={() =>
